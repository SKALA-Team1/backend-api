...
class SessionManager:
    ...
    def __init__(self):
        ...
        self._session_locks: Dict[str, asyncio.Lock] = {}  # ✅ 세션별 Lock
    ...
    def append_message(
        ...
        """
        대화 히스토리에 메시지 추가 (동기 버전, 하위 호환성)

        주의: 이 메서드는 race condition에 취약합니다.
        새로운 코드에서는 append_message_async() 사용 권장.
    ...
    async def increment_utterance_index_async(self, session_id: str) -> int:
        """
    ...
    def increment_utterance_index(self, session_id: str) -> int:
        """
        발화 인덱스 증가 (동기 버전, 하위 호환성)

        주의: 이 메서드는 race condition에 취약합니다.
        새로운 코드에서는 increment_utterance_index_async() 사용 권장.
