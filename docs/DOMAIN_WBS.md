...
SKALA는 AI 기반 실시간 영어 회화 연습 플랫폼입니다. 다음 3개의 도메인으로 구성됩니다:

1. **Roleplaying Domain** (역할극 도메인) - 핵심 기능
2. **Textbook Domain** (교재 도메인) - 교재 기반 학습
3. **User Domain** (사용자 도메인) - 사용자 관리 및 통계
...
## 2. TEXTBOOK DOMAIN (교재 도메인)

### 2.1 레슨 관리 (Lesson Management)
...
**API 엔드포인트:**
- [ ] `GET /textbook/textbooks` - 교재 목록
- [ ] `GET /textbook/textbooks/{textbookId}/lessons` - 레슨 목록
- [ ] `GET /textbook/lessons/{lessonId}` - 레슨 상세
- [ ] `POST /textbook/lessons/{lessonId}/start` - 레슨 시작
- [ ] `POST /textbook/lessons/{sessionId}/submit-answer` - 답변 제출
- [ ] `POST /textbook/lessons/{sessionId}/finish` - 레슨 종료
- [ ] `GET /textbook/lessons/{sessionId}/review` - 리뷰
...
## 3. USER DOMAIN (사용자 도메인)

### 3.1 사용자 정보 관리 (User Profile)
...
| Textbook | 레슨 관리 | ❌ 미구현 | ❌ 미구현 | 🟢 낮음 |
| Textbook | 질문/답변 | ❌ 미구현 | ❌ 미구현 | 🟢 낮음 |
...
### Phase 3: Textbook & User 도메인 - **우선순위 낮음**
